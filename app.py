...
配電盤(受変電キュービクル)の外形図・盤姿図が読める場合、その盤に "outline" を付ける:
  "outline": {"faces": 面数(区分/連の数=キュービクルの縦仕切り数の整数), "w": 幅mm, "h": 高mm, "d": 奥行mm}
- 面数(faces): 外形図で盤が何面(何区分/何連)に分かれているかの数。1面なら1、二段積や横並びで6区分なら6。
  盤名に「○面」「○連」と明記があればその数。読み取れない項目は省略(faces/w/h/d のうち分かる物だけ)。
- サイズ(w/h/d): 全体または該当盤の外形寸法(mm)。W2100×H2300×D1000 等の表記を数値で。
※この outline は配電盤面あたりの照明員数(1/面)や函体・段積の判断に使う。制御盤/分電盤には不要。
...
def select_from_extracted(data):
    ...
    for p in data.get('panels',[]):
        ...
        if isinstance(_ol2,dict) and _ol2:
            ...
            _og['outline_note']='外形図: %s面%s'%(_ol2.get('faces','?'), ' '+_sz if _sz else '')
        out.append(_og)
