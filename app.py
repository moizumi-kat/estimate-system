...
【外形図・盤外形の艤装品（換気扇・盤内照明・スペースヒーター等）も拾う】
盤の外形図(外形寸法図)や盤面図に、盤の艤装品として「換気扇(ベンチレータ)」「盤内照明(FL/蛍光灯)」
「スペースヒーター」「サーモスタット」が描かれていたら、その盤の機器として抽出する
(例 name="換気扇", name="盤内照明", name="スペースヒーター")。
これらは盤を冷却/照明/保温する艤装品であり、モーター負荷の換気扇(FE-1/FS-1等のID・3φ・kW・遮断器付)
とは別物。艤装品の換気扇は負荷IDやkW表記を持たない。負荷の換気扇は従来どおり分岐回路の負荷として拾う。

【配電盤(受変電)の外形図から「面数」と「サイズ」を拾う】
配電盤(受変電キュービクル)の外形図・盤姿図が読める場合、その盤に "outline" を付ける:
  "outline": {"faces": 面数, "w": 幅mm, "h": 高mm, "d": 奥行mm, "stories": 段数(1/2/3),
              "sections":[{"name":"盤名","tr_kva":TR容量, "fan": 換気扇有無true/false}]}
- 面数(faces): 外形図で盤が何面(何区分/何連)に分かれているかの数。正面図下端の盤名ラベルの数。
- 段数(stories): 変圧器が縦に積まれている段数(側面図/正面図で判断)。単独=1、二段積=2、三段積=3。既定1。
- サイズ(w/h/d): 全体外形寸法(mm)。W14100×H2400×D2500 等を数値で。読めない項目は省略。
- sections: 各面の盤名・TR容量・換気扇有無(上部/扉のルーバー=換気扇)。分かる範囲で。
※outlineは 照明員数(=面数)・換気扇(=外形図の換気扇数)・段積(16系,段数)・函体寸法の判断に使う。制御盤/分電盤には不要。
...
def _panel_fan_code(name, panel=''):
    """外形図に描かれた『盤用換気扇』(盤の冷却ファン)を盤種別コードで拾う(茂泉様ルール:先ず外形図どおり)。
    配電盤=屋外型74102 / 制御盤=制御盤用74103 / その他=74100。
    負荷の換気扇(FE/FS等のID・3φ・kW・遮断器付=モーター負荷)は対象外(分岐遮断器で別途拾う)。"""
    s=unicodedata.normalize('NFKC',str(name))
    if not re.search(r'換気扇|ﾍﾞﾝﾁﾚｰﾀ|ベンチレータ|盤用ファン|盤内ファン|パネルファン', s): return None
    # 負荷(モーター)の換気扇は除外: kW/相/遮断器/負荷ID(FE-1等)を伴うものは負荷回路
    if re.search(r'\d\.?\d*\s*kw|[0-9０-９]\s*[φΦ]|(?<![A-Za-z])3\s*[φΦ]|MCCB|MCB|ELB|ELCB|[1-4]\s*[PＰ](?![a-z])|\d+\s*A[FT]|[A-Za-z]{1,3}[-‐]\d', s, re.I):
        return None
    kind=_panel_kind(panel)
    code='74102' if kind=='haiden' else ('74103' if kind=='ctrl' else '74100')
    if code in byCode: return code,'○','盤用換気扇(外形図の記載どおり計上・盤種既定=%s)'%byCode[code].get('name','')[:14]
    return None
...
def select_one(name, panel='', prev_is_main=False, volt='', symbol='', kw='', group='', legend=None, breaker=''):
    ...
    # 外形図に描かれた盤用換気扇(盤冷却ファン)は盤種別コードで拾う(負荷の換気扇は除外)。
    _pf=_panel_fan_code(name, panel)
...
def _std_accessories(panel_name, rows, cold=False, faces=1):
    """茂泉様の艤装ルールで盤面の標準付属品[(code,qty)]を返す(自動計上分)。
      照明: 配電盤(受変電)一面に71091(FL-10WﾄﾞｱSW付)×1。面数(faces)が外形図から分かれば×面数。制御盤/分電盤は無し。
    ...
    out=[]
    _nf=max(1,int(faces or 1))
    if '71091' in byCode: out.append(('71091',_nf))               # 照明1/配電盤面 × 面数
    # 外形図に換気扇が描かれ既に計上済(74100-74103)なら、その記載を優先しTR≥500の自動追加はしない(二重計上防止)。
    _has_fan=any(str(r.get('code','') or '') in ('74100','74101','74102','74103') for r in rows)
    tr500=False
    for r in rows:
    ...
    if tr500 and not _has_fan and '74102' in byCode: out.append(('74102',1))   # TR≥500kVA盤の換気扇(外形図に無い時のみ)
...
def select_from_extracted(data):
    ...
    for p in data.get('panels',[]):
        ...
        if isinstance(_ol2,dict) and _ol2:
            _og['outline']=_ol2
            _sz='×'.join('%s%d'%(k.upper(),_ol2[k]) for k in ('w','h','d') if _ol2.get(k))
            _st=int(_ol2.get('stories') or 1)
