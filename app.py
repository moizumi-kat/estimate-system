...
# ============================================================================
# 検図（図面不具合チェック）: 設計次工程で走らせるQCエンジンを Web で提供する。
#   POST /api/kenzu  … DXF(複数/ZIP可)を受け取り、系統を自動仕分けして R1-R7+H1 を実行
#   GET  /kenzu      … 簡易アップロード画面
# 判定は設計（確認ゲート）。出力は {場所/問題/提案/根拠} 様式。
# ============================================================================
def _kenzu_models(files):
    """アップロードされた werkzeug FileStorage 群→[(filename, DrawingModel)]。
    ZIP は展開。DXF 以外は警告に回す。一時ファイル経由(ezdxf はパス読込)。"""
    from wireharness.fromto_qc.geometry import DrawingModel
    models=[]; warnings=[]; tmps=[]
    def _one(fname, raw):
        try:
            with tempfile.NamedTemporaryFile(suffix='.dxf', delete=False) as tf:
...
def api_kenzu():
    ...
    from wireharness.fromto_qc import run_check
    ...
    try:
        ...
        seq=buckets['seq']; skel=buckets['skel']
        layout=buckets['layout']; table=buckets['table']
        # run_check.run はパスを取るのではなくモデルを受ける版が無いため、モデル直渡し用に薄く呼ぶ
        ...
        counts={}
        for f in uniq: counts[f.get('rule','?')]=counts.get(f.get('rule','?'),0)+1
...
def _kenzu_run(seq, skel, layout, table, ai=False):
    """run_check.run のモデル直渡し版(パス→モデル変換を省く)。ルール構成は run_check と一致。"""
...
@app.route('/kenzu')
@login_required
def kenzu_page(): return Response(KENZU_HTML, mimetype='text/html')

KENZU_HTML="""<!DOCTYPE html><html lang=ja><head><meta charset=UTF-8>
<meta name=viewport content="width=device-width,initial-scale=1"><title>図面検図</title>
<style>
body{font-family:'Yu Gothic',Meiryo,sans-serif;background:#f7f5ef;margin:0;color:#26332b}
.wrap{max-width:960px;margin:0 auto;padding:20px 16px 60px}
h1{font-size:19px;color:#1e3a28;margin:0 0 4px}.sub{color:#6b7a70;font-size:13px;margin:0 0 18px}
...
a.back{font-size:12px;color:#5c6b60;text-decoration:none}
</style></head><body><div class=wrap>
<a class=back href="/">← コード選定へ</a>
<h1>図面検図（設計次工程チェック）</h1>
